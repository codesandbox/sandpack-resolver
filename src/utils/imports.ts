...
type PackageImportType = string | null | false | PackageImportObj | PackageImportArr;
...
export function extractSpecifierFromImport(
  ...
  if (Array.isArray(importValue)) {
    const foundPaths = importValue.map((v) => extractSpecifierFromImport(v, pkgRoot, importKeys)).filter(Boolean);
